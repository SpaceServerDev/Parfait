...
class Ore{
	...
	public function canPlaceObject(ChunkManager $level, int $x, int $y, int $z) : bool{
		return $level->getBlockIdAt($x, $y, $z) === Block::STONE or $level->getBlockIdAt($x, $y, $z) === Block::END_STONE or $level->getBlockIdAt($x, $y, $z) === Block::TERRACOTTA or $level->getBlockIdAt($x, $y, $z) === Block::PRISMARINE;
	}
						...
						if($yy > 0 and ($sizeX + $sizeY) < 1){
							for($zz = $startZ; $zz <= $endZ; ++$zz){
								...
								if(($sizeX + $sizeY + $sizeZ) < 1){
									if($level->getBlockIdAt($xx, $yy, $zz) === Block::STONE or $level->getBlockIdAt($xx, $yy, $zz) === Block::TERRACOTTA or $level->getBlockIdAt($xx, $yy, $zz) === Block::END_STONE or $level->getBlockIdAt($xx, $yy, $zz) === Block::PRISMARINE){
